...
def runPipeline(image, llrobot):
    ...
    lower_target = np.array([int(LOWER_HSV_PERCENT[0] / 2), int(LOWER_HSV_PERCENT[1] * 2.55), int(LOWER_HSV_PERCENT[2] * 2.55)])
    upper_target = np.array([int(UPPER_HSV_PERCENT[0] / 2), int(UPPER_HSV_PERCENT[1] * 2.55), int(UPPER_HSV_PERCENT[2] * 2.55)])
    ...
    if len(contours) > 0:
        ...
        for contour in contours:
            ...
            if (w > MIN_TARGET_WIDTH_PX and h > MIN_TARGET_HEIGHT_PX and w < MAX_TARGET_WIDTH_PX and h < MAX_TARGET_HEIGHT_PX):
